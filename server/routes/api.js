...
exports.fetchMovies = function(req, res) {
  res.status(200).json(uglifyResponse(movies));
};
...
exports.fetchMovie = function(req, res) {
  ...
  res.status(404).json(uglifyResponse("Not found"));
};
...
exports.fetchMovieImages = function(req, res) {
  ...
  for (var i = 0; i < movies.length; i++) {
    if (movies[i].id == id) {
      res.status(200).json(uglifyResponse({poster : movies[i].poster}));
    }
  }
...
exports.fetchMovieInformations = function(req, res) {
  ...
  for (var i = 0; i < movies.length; i++) {
    if (movies[i].id == id) {
      res.status(200).json(uglifyResponse({
...
exports.searchMovieByTitle = function(req, res) {
  ...
  if (title && results.length) {
    res.status(200).json(uglifyResponse(results));
  }
  res.status(404).json(uglifyResponse("Not found"));
};
...
exports.addMovie = function(req, res) {
  ...
  for (var idx in movies) {
    if (movies[idx].title === movie.title) {
      res.status(500).json(uglifyResponse({error : 'Le film ' + movie.title + ' a déjà été ajouté.'}));
      err = true;
    }
  }
  ...
  if (!err) {
    ...
    movies.push(movie);
    res.status(201).json(uglifyResponse(movie));
  }
...
exports.updateMovie = function(req, res) {
  ...
  res.status(304).json(uglifyResponse("Not modified"));
};
